...
namespace PlantUmlLanguageService.Services.Remote
{
    internal class RemoteDiagramService : IDiagramService
    {

        /// <summary>
        /// The plant uml URL format
        /// </summary>
        private readonly string _plantUmlUrlFormat;

        public RemoteDiagramService(string plantUmlRemote = "https://www.plantuml.com/plantuml")
        {
            _plantUmlUrlFormat = plantUmlRemote + "/{0}/{1}";
        }
        ...
        /// <returns></returns>
        public string GetImageUrlForSource(string source, string imageFormat)
        {
            if (!string.IsNullOrEmpty(source))
            {
                return string.Format(_plantUmlUrlFormat, imageFormat, Encode64(ZipStr(System.Web.HttpUtility.UrlDecode(System.Web.HttpUtility.UrlEncode(source).Replace("+", "%20")))));
            }
            return Constants.NoImageBase64;
        }


        public async Task<Validator> GetValidator(string uri)
        {
            Validator validator;
            ...
            try
            {
                validator = new Validator(response.Get("X-PlantUML-Diagram-Description"), response.Get("X-PlantUML-Diagram-Error"), response.Get("X-PlantUML-Diagram-Error-Line"));
            }
            catch
            {
